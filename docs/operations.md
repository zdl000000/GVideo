...
```powershell
docker compose exec -T backend gvideo data-grant-admin <username>
```

管理员身份自 v2 迁移起存储为 `users.is_admin` 持久标志；`ADMIN_USERNAME` 只提供首个注册入口，一旦存在管理员，保留名即锁定。以下情况应使用该命令显式授予：老库升级后管理员标志缺失、数据合并后需要恢复管理员、或管理员账号变更后需要重新指定。命令对不存在的用户名返回错误，对同一用户重复授予是幂等的，授予后该用户下一次请求即生效，无需重启服务。
