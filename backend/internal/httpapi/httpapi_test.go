...
func TestCreatorSpaceAndFollowingHTTPFlow(t *testing.T) {
	...
	handler := New(service.New(repo, cfg, logger), moderation.NewService(moderation.NewRepository(db)), notifications.NewService(notifications.NewRepository(db)), comments.NewService(comments.NewRepository(db), logger), interactions.NewService(interactions.NewRepository(db), logger), cfg, logger).Routes()
...
func TestNotificationEndpointsAuthenticationCSRFAndOwnership(t *testing.T) {
	...
	cfg := config.Config{MediaDir: filepath.Join(dir, "media"), SessionTTL: time.Hour}
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	handler := New(service.New(repo, cfg, logger), moderation.NewService(moderation.NewRepository(db)), notifications.NewService(notificationRepo), comments.NewService(comments.NewRepository(db), logger), interactions.NewService(interactions.NewRepository(db), logger), cfg, logger).Routes()
