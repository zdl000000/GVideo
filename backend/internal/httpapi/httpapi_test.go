...
func TestCreatorSpaceAndFollowingHTTPFlow(t *testing.T) {
	...
	handler := New(service.New(repo, cfg, logger), moderation.NewService(moderation.NewRepository(db)), notifications.NewService(notifications.NewRepository(db)), comments.NewService(comments.NewRepository(db), nil, logger), interactions.NewService(interactions.NewRepository(db), nil, logger), cfg, logger).Routes()
...
func TestDeleteCommentHTTPAuthorization(t *testing.T) {
	...
	comment, err := comments.NewRepository(db).CreateComment(context.Background(), author.User.ID, video.ID, "delete through HTTP")
	if err != nil {
		t.Fatal(err)
	}
...
func TestFavoriteVideosEndpoint(t *testing.T) {
	...
	if _, err := interactions.NewRepository(db).ToggleFavorite(context.Background(), auth.User.ID, video.ID); err != nil {
		t.Fatal(err)
	}
...
func TestNotificationEndpointsAuthenticationCSRFAndOwnership(t *testing.T) {
	...
	cfg := config.Config{MediaDir: filepath.Join(dir, "media"), SessionTTL: time.Hour}
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	handler := New(service.New(repo, cfg, logger), moderation.NewService(moderation.NewRepository(db)), notifications.NewService(notificationRepo), comments.NewService(comments.NewRepository(db), nil, logger), interactions.NewService(interactions.NewRepository(db), nil, logger), cfg, logger).Routes()
	...
	if err := notificationRepo.Create(ctx, recipient.User.ID, actor.User.ID, "like", video.ID, 0, video.Title, ""); err != nil {
		t.Fatal(err)
	}
	...
	if err := notificationRepo.Create(ctx, recipient.User.ID, actor.User.ID, "favorite", video.ID, 0, video.Title, ""); err != nil {
		t.Fatal(err)
	}
	result = httptest.NewRecorder()
