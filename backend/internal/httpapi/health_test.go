...
func TestReadyzReportsOnlyDatabaseReadiness(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			routes := New(nil, nil, cfg, logger).WithReadiness(test.checker).Routes()
			response := httptest.NewRecorder()
	...
	New(nil, nil, cfg, logger).Routes().ServeHTTP(notAllowed, httptest.NewRequest(http.MethodPost, "/readyz", nil))
